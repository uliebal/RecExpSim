...
def myresponse(x, y, nx, ny):
    ...
    A = biocircuits.aa_and(x, y, nx, ny) # first term of [(A and B) or (not A and not B)]
    B = biocircuits.rr_and(x, y, nx, ny) # second term of [(A and B) or (not A and not B)]
    return (A**nx + B**nx) / (1 + A**nx + B**nx)
